...
for (let i = 0; i < parsedCircuit[0].length; ++i) {
    ...
    const p = parsedCircuit[0][i]
    if (binaryFind(p, parsedCircuit[1])) {
        intersect.push(p)
    }
}
//const intersect = parsedCircuit[0].filter(p => binaryFind(p, parsedCircuit[1]))
let sorted = intersect.sort((a, b) => a.manhattan() - b.manhattan()).reverse()
console.log(sorted.slice(-1)[0].manhattan())