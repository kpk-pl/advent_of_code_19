...
const assembly = {
    ...
    2: { operation: "mul", length: 4 },
    3: { operation: "read", length: 2 },
    4: { operation: "write", length: 2 },
    99: { operation: "halt", length: 1 },
}

function OpCode(code) {
    code = parseInt(code)
    this.operation = assembly[code % 100]
    code = Math.floor(code / 100)

    this.modes = []
    for (let i = 0; i < this.operation.length - 1; ++i) {
        switch (code % 10) {
            case 0:
                this.modes.push("position")
                break
            case 1:
                this.modes.push("immediate")
                break
        }
...
function ExecutionContext(pc, code) {
    this.pc = pc
    this.code = code

    this.addressOf = function (arg) {
        switch (this.code.modes[arg]) {
            case "position":
                return program[this.pc + arg + 1]
            case "immediate":
                return this.pc + arg + 1
        }
    }
...
function Executor() {
    ...
    this.op_add = function (ctx) {
        program[ctx.addressOf(2)] = program[ctx.addressOf(0)] + program[ctx.addressOf(1)]
    }
    this.op_mul = function (ctx) {
        program[ctx.addressOf(2)] = program[ctx.addressOf(0)] * program[ctx.addressOf(1)]
    }
    this.op_read = function (ctx) {
        program[ctx.addressOf(0)] = input.shift()
    }
    this.op_write = function (ctx) {
        output.push(program[ctx.addressOf(0)])
    }
    this.op_halt = function () {
        this.done = true
    }

    this.executeOne = function () {
        const code = new OpCode(program[this.pc])
        console.log(code)
        const ctx = new ExecutionContext(this.pc, code)
