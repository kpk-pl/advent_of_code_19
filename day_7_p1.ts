    ...
    for (let phase = 1; phase <= 4; ++phase) {
        ...
        for (let i = 0; i <= phase; ++i) {
            for (let perm of base) {
                let newPerm: number[] = [...perm.slice(0, i), phase, ...perm.slice(i)]
                result.push(newPerm)
            }
        }
    }
...
for (let phases of allPhases()) {
    ...
    for (let phase of phases) {
        let input = [phase, amplification]
        let executor = new Executor(program, input)
        executor.executeAll()
        amplification = executor.output[0]
    }
