...
function Executor() {
    ...
    this.op_add = function (ctx) {
        program[ctx[2]] = program[ctx[0]] + program[ctx[1]]
    }
    this.op_mul = function (ctx) {
        program[ctx[2]] = program[ctx[0]] * program[ctx[1]]
    }
    this.op_read = function (ctx) {
        program[ctx[0]] = input.shift()
    }
    this.op_write = function (ctx) {
        output.push(program[ctx[0]])
    }
    this.op_halt = function (ctx) {
        this.done = true
    }
    this.op_jump_if = function (ctx) {
        if (program[ctx[0]] > 0)
            this.pc = program[ctx[1]] - ctx.code.operation.length
    }
    this.op_jump_zero = function (ctx) {
        if (program[ctx[0]] == 0)
            this.pc = program[ctx[1]] - ctx.code.operation.length
    }
    this.op_lt = function (ctx) {
        program[ctx[2]] = (program[ctx[0]] < program[ctx[1]])
    }
    this.op_eq = function (ctx) {
        program[ctx[2]] = (program[ctx[0]] == program[ctx[1]])
    }
