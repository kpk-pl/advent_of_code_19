...
function Point(x, y, distance) {
    ...
    this.next = function (direction) {
        switch (direction) {
            case 'L':
                return new Point(this.x - 1, this.y, this.distance + 1)
            case 'R':
                return new Point(this.x + 1, this.y, this.distance + 1)
            case 'D':
                return new Point(this.x, this.y - 1, this.distance + 1)
            case 'U':
                return new Point(this.x, this.y + 1, this.distance + 1)
        }
    }
...
function binaryFind(point, array) {
    while (array.length > 0) {
        let midpoint = Math.floor(array.length / 2)
        let eq = point.less(array[midpoint])
        if (eq == 0)
...
function laySegment(segment, start) {
    ...
    for (let i = 0; i < length; ++i) {
        ...
        result.push(last.next(direction))
    }
...
function layout(wire) {
    ...
    for (let segment of wire) {
        result = result.concat(laySegment(segment, result.slice(-1)[0]))
    }

    return result.slice(1).sort((a, b) => a.less(b));
}

function parseWire(input) {
    return input.split(',')
}

const parsedCircuit = circuit.map(el => layout(parseWire(el)))

let intersect = []
for (let i = 0; i < parsedCircuit[0].length; ++i) {
    if (i % 10000 == 0)
        console.log(`${i}/${parsedCircuit[0].length}`)
    const p1 = parsedCircuit[0][i]
    const p2 = binaryFind(p1, parsedCircuit[1])
    if (p2 !== undefined)
        intersect.push([p1, p2])
}

function combinedDist(left, right) {
    return left.distance + right.distance
}

let sorted = intersect.sort((a, b) => combinedDist(...a) - combinedDist(...b)).reverse()
console.log(combinedDist(...sorted.slice(-1)[0]))